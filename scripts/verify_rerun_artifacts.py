...
def main() -> int:
    ...
    if not {"tune_baseline", "tune_candidate", "eval"}.issubset(set(runs["phase"].unique())):
        fail("runs_long.csv must contain tune_baseline, tune_candidate, and eval phases")
    ...
    if cell_stats.empty:
        fail("cell_stats.csv must not be empty")
    ...
    if method_agg.empty:
        fail("method_aggregate.csv must not be empty")
